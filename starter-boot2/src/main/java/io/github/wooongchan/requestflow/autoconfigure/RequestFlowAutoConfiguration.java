...
/**
 * Spring Boot 2.6 전용 어댑터 — starter-boot3의 동명 클래스와 빈 구성은 동일하고, 등록 방식만 다르다.
 * Boot 2.7+에서 생긴 {@code @AutoConfiguration}/{@code AutoConfiguration.imports} 대신, 2.6까지 쓰던
 * 방식대로 {@code @Configuration} + {@code META-INF/spring.factories}로 등록한다.
 */
@Configuration
...
public class RequestFlowAutoConfiguration {
    ...
    public Advisor requestFlowTraceAdvisor(ValueSerializer valueSerializer) {
        Pointcut pointcut = new RequestFlowPointcut(
                properties.getBasePackages().stream().map(String::trim).collect(Collectors.toList()));
